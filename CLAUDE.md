...
- `src/CtrlAgent.Gui` — Avalonia desktop app (Fluent dark theme, compiled bindings). `HostEngine` mirrors the console host's two background loops and raises events; `MainViewModel` marshals them onto the UI thread via `Dispatcher.UIThread.Post`. Avalonia is pinned to the last 11.x (11.3.18); the host loops in `HostEngine` intentionally parallel `App/Program.cs` — keep behavior changes in sync between the two until they share a hosting layer.
