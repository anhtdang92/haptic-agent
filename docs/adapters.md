...
- Verified live against Claude Code CLI 2.1.150 (2026-07-23): `system/init` (session id), `assistant`, and `result` shapes match; the flag set including `--permission-prompt-tool stdio` is accepted. The CLI also emits `system` subtypes the parser ignores (e.g. `api_retry`); an authenticated approval-flow run is still outstanding.
