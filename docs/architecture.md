...
- The composer sits **inside the conversation card**, not docked to the window: it is the bottom of the chat column (box on top, action row beneath — the column is narrower than the window, so a side-by-side row squeezed the box into a slot), with the attachments strip and the approval banner stacked above it in the same card. The prompt box is multi-line (pasted stack traces, numbered steps). Enter submits via a **tunneling** `KeyDown` handler — with `AcceptsReturn` the TextBox consumes Enter and marks it handled, so a bubbling handler or `KeyBinding` never sees it — Shift+Enter inserts a newline, and sending clears the box. A composer mic button runs one `SpeechToTextService` dictation pass into the box for review (never auto-sent); clicking again cancels. Interrupt rides the action row icon-only while a turn is live.
