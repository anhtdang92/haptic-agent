...
- **Both windows draw shortcuts in one language.** The mission-control hub list and its F1 overlay render the same `ChordToken` chips as Mainframe's HUD, and the hub is sized so the whole available set fits at the default window size (the minimum-width window scrolls; F1 is the full map there). Mainframe's header also carries minimize and fullscreen/windowed controls — windowed mode restores system decorations so the OS supplies move/resize.
- **The chip language follows the pad.** `ChordToken` renders chords as physical parts — face buttons as colored circles, the d-pad as a drawn cross with its arm lit (`DPadIcon`), everything else as keycap text. Its static `PlayStation` flag flips on `ControllerConnected` *before* the binding rows re-mint, and with it face chips become Sony's shapes in Sony's colors (positional: A→✕, B→○, X→□, Y→△) as stroked geometry — the bundled font has no shape glyphs. Prose goes through `ControlLabels.Label`, which says "Cross"/"Triangle" on a DualSense, so the bot's coaching, the Mainframe legend, and the voice-overlay instructions never tell a Sony player to press a button their pad does not have.
- **The main window is mission control; only Mainframe codes** (owner's product call). Mission control carries the hero/status card and session knobs, an Enter-Mainframe call-to-action, the severity-tinted EVENTS stream, and the controller hub — no conversation, composer, sessions rail, or diff panel. Its tool rail holds new-session, Mainframe, overlay, mic picker, profile editor, and the validation wizard; its keyboard is F1 (shortcuts overlay), Ctrl+N, F11. `MainViewModel` still owns all shared state (transcript, prompt text, diff rows, sessions) because Mainframe renders through it — the split moved XAML, not logic. A voice or attach request from anywhere opens Mainframe: dictation runs in its overlay, attachment chips render in its composer.
...
- **Reading history beats following it.** The event stream and the Mainframe feed follow new items only while already scrolled to the bottom (`MainWindow.StickToBottom`, `FollowFeedIfAtBottom`); during a busy turn events arrive several times a second, and unconditional auto-scroll made the history unreadable exactly when it mattered.
- Mainframe's composer is the only composer: prompt box above an action row, multi-line (pasted stack traces, numbered steps). Enter submits via a **tunneling** `KeyDown` handler — with `AcceptsReturn` the TextBox consumes Enter and marks it handled, so a bubbling handler or `KeyBinding` never sees it — Shift+Enter inserts a newline, and sending clears the box.
- **Session resume** lives in Mainframe's Sessions screen: `MainViewModel.RefreshSessions` lists the workspace's sessions via `ClaudeSessionCatalog` (off the UI thread, swapped in on it), resume goes through `HostEngine.ResumeSessionAsync` and reloads the stored conversation, and the list refreshes when the session changes or a turn ends. Available only when the adapter's store is readable (today: Claude).
...
- **Diff review panel** (Mainframe's "Workspace changes" tile, F3): a fullscreen overlay of the workspace's pending changes — tracked edits from `git diff HEAD`, untracked files folded in as all-added synthetic diffs — as colored add/remove rows with per-file headlines and a "+X −Y" summary. The parsing (`UnifiedDiffParser`), collection (`WorkspaceDiff`), and row assembly (`DiffPanel.Build` — the flat row list, the elision cap, and one `DiffAnchor` per rendered file header) live in `CtrlAgent.Presentation` where they are unit-tested; `DiffRow` in the GUI only picks colors. A file rail (shown for 2+ files) jumps to any file and highlights the current one — clicks and the d-pad's `StepDiffFile` both land on the same `DiffAnchor`s, and jumps measure the target row's real position (rows vary in height — index arithmetic would drift) — while the right stick pages lines. B/Esc closes. The card is opaque, unlike every other `.card` — code is read, not glanced at.
...
- `ProfileEditorWindow` edits bindings with live `ControllerProfileValidator` feedback and applies via `HostEngine.TryApplyProfile`. Its combos store enum names (the JSON vocabulary) but display humanized labels — "Submit prompt", "LB · LeftShoulder" — and the modifiers field parses pad language ("LB+RB", "P1", "Cross", "D-Up") through `ControlShorthand` in `CtrlAgent.Presentation` (unit-tested; formal enum names still accepted, so pasted JSON fragments keep working). `GuiSettings` persists last-used launch options to `%AppData%/CtrlAgent`; a first-run setup overlay collects agent + working directory so no CLI flags are needed.
...
13. **CtrlAgent is a control surface, not an editor — by explicit product decision.** "Pure vibe coding": the user directs and judges, the agent types. Nothing in the app edits code, and nothing should — the workspace is the user's real folder, their real editor already has it open, and a half-good embedded editor would compete with a fully-good external one while diluting the one thing this app is for. The test for any proposed feature: does it help the user *direct or judge* the agent (yes: prompts, voice, diffs, approvals, session control) or does it help them *type code* (no).
14. **The diff panel asks git, not a library.** The whole product is answering approvals from a controller, and until the panel existed the approve button was reachable while the thing being approved was visible nowhere. `WorkspaceDiff` shells out to the user's own `git` (`diff HEAD` for tracked changes — the index is irrelevant to "what would I be approving" — plus `ls-files --others` for untracked files, folded in as synthetic all-added diffs) so the answer always matches what their own `git status` says; `UnifiedDiffParser` turns the output into records and is pure, so the folding rules sit under unit tests with mutation checks. Trade-off: no diff without git on PATH, reported as a readable error rather than hidden.
15. **Only Mainframe codes; the main window is mission control — by explicit product decision.** The two windows briefly both carried a full coding surface (chat, composer, sessions, diff), which meant two of everything: two conversations to keep scrolled, two approval surfaces, two places a prompt could be half-typed. The owner's call resolved it: Mainframe is *the* place work happens, and the main window sets up, configures, tests the controller, and watches diagnostics. The shared `MainViewModel` made the split cheap — Mainframe always rendered through it, so stripping the main window's work surfaces moved XAML, not logic, and nothing about the engine or adapters changed. Voice and attach requests from mission control open Mainframe rather than growing local UI for them.
