...
| `modifiers` | string[] | none | Controls that must be held for this binding to match (turns it into a chord). JSON uses the `ControllerControl` names above; the in-app profile editor additionally accepts pad shorthand (`LB`, `RB`, `LT`, `RT`, `LS`, `RS`, `P1`–`P4`, `Cross`/`Circle`/`Square`/`Triangle`, `D-Up`…) |
