...
| 2 | Live verification of Codex + Claude Code adapters | Wire shapes follow documented protocols; first real run is the compatibility test | **Claude Code swept live 2026-07-26** (2.1.220: init, streaming turns, approve/deny/approve-for-session, interrupt, all five permission modes, model, compact, crash restart — found and fixed per-turn init spam; session cycling still needs a non-nested shell). **Verified 2026-07-24** (CLI 2.1.150: session init, prompt turns, controller-answered approve and deny, error events; found and fixed the Windows bare-name spawn bug — see `docs/adapters.md`). Codex still blocked on an installed CLI; its wire handling is now extracted into the pure, unit-tested `CodexProtocolParser`, so the live run is a compatibility check rather than a first look at the logic |
...
| 6 | Release packaging: self-contained win-x64 zip (App + Gui + bridge) on tags | Install without a dev environment | **Done** — release workflow on `v*` tags builds the portable zip **and an Inno Setup installer** (per-user, Start Menu, optional desktop/startup shortcuts, uninstaller). Installer untested until the first tag fires the workflow |
| 7 | Haptic pattern tuning pass | Depends on #1 evidence; **every value in `HapticPatternCatalog` is an unverified estimate** and the cues are the product | Blocked on #1. Also the right moment to add a distinct interrupt cue — interrupts currently share the completion cue (see `AgentInterrupt`) |
