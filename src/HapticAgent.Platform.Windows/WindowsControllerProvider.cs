...
public sealed class WindowsControllerProvider : IControllerProvider
{
    ...
    private static string? ResolveBridgePath(string? explicitPath)
    {
        ...
        var environmentPath = Environment.GetEnvironmentVariable("HAPTIC_AGENT_GAMEINPUT_BRIDGE");
        if (!string.IsNullOrWhiteSpace(environmentPath))
        {
            return Path.GetFullPath(environmentPath);
        }

        var adjacent = Path.Combine(AppContext.BaseDirectory, "HapticAgent.GameInputBridge.exe");
